...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    xdg.configFile = {
      ...
      # Hyprland Config — upstream migrated to a Lua-based config in May 2026 (Hyprland 0.55+).
      # Entry point is now hyprland.lua. Hyprland auto-loads it when no hyprland.conf is present.
      # We mirror upstream's hyprland.lua and append plugin loading at the end.
      ...
      '' + lib.concatMapStrings (plugin: ''
        hl.plugin.load("${plugin}/lib/lib${plugin.pname}.so")
      '') cfg.hyprland.plugins;

      # Hyprland Environment — generated env.lua overrides upstream's hyprland/env.lua
      # (which is what `require("hyprland.env")` in hyprland.lua loads).
      # We need this to inject Nix-aware paths (XDG_DATA_DIRS, QT plugin paths, qsConfig, etc.)
      ...
      # Symlink other hyprland lua modules and assets
      ...
      # Custom (user-override) lua siblings — upstream now ships these as .lua too
      ...
      # Note: monitors.conf and workspaces.conf were removed from upstream;
      # nwg-displays will generate them on demand if installed.
